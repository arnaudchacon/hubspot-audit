...
// renders them as three sections — fallback should be indistinguishable.
export const FALLBACK_RECOMMENDATIONS: Record<string, string> = {
  duplicates:
    'ROOT CAUSE: Most duplicate contact problems trace back to a single intake path that never had a dedup gate — usually a form that fires on every submission without checking if that email already exists. The duplicates don\'t appear all at once; they compound quietly for months.\n\nFIRST STEP: Pull your contacts list, filter by email domain, and sort by create date. Clusters of the same domain created days apart are your duplicates. On Pro/Enterprise, Settings → Data Management → Duplicates surfaces the highest-confidence matches. On Free/Starter, you\'re doing this manually in a spreadsheet — export, sort on email, merge row by row.\n\nWATCH FOR: The trap most people fall into is merging without fixing the intake. The duplicates come back within weeks. Find out which form or import triggered the pattern first.',

  owners:
    'ROOT CAUSE: Orphaned contacts almost always come from one of two things: a CSV import that didn\'t map the owner column, or a form that has no default owner rule. The age distribution of your unowned contacts tells you which one — if they\'re recent, the intake is still broken right now.\n\nFIRST STEP: Filter your contacts list by "Contact owner is unknown," bulk-select, and use the Assign action. On Starter and above, set a default owner in your form settings or wire a simple Workflow: Contact Created → Set Owner. Free tier users get no automation here — it has to be manual.\n\nWATCH FOR: Don\'t skip the root cause step. If you assign owners today without fixing the intake, you\'ll be doing this again in 90 days.',

  workflows:
    'ROOT CAUSE: Active workflows with no recent enrollments are usually campaigns or events that ended without anyone archiving them. They\'re not doing damage, but they count against your workflow limit and make it impossible to audit what\'s actually running.\n\nFIRST STEP: Go to Automation → Workflows, filter by Active, sort by enrollment count ascending. Anything at zero for 30+ days should be paused — not deleted. Deletion removes the enrollment history, which sometimes matters for attribution.\n\nWATCH FOR: Pausing a workflow doesn\'t unenroll contacts already in it. Check the "Currently enrolled" number before you pause — if it\'s non-zero on a workflow you think is dead, something is still triggering it.',
